...
import campaignsIcon from '../icons/campaigns.png';
import accountsIcon from '../icons/accounts.png';
import inboxIcon from '../icons/master-inbox.png';
import clientAccessIcon from '../icons/clientAccess.png';
import integrationIcon from '../icons/integration.png';
import leadManagementIcon from '../icons/lead-management.png';
import analyticsIcon from '../icons/global-analytics.png';
import supportIcon from '../icons/help-support.png';
      ...
      <div className="flex-grow overflow-y-auto mt-6">
        {menuItems.map((item, index) => (
          <Link
            key={index}
            to={item.path}
            className={`flex items-center ${isOpen ? 'px-6' : 'justify-center'} py-4 text-white hover: transition-colors duration-200n hover:text-black ${location.pathname === item.path ? 'text-[#684FFF]' : ''}`}
          >
